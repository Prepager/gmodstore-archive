...
local function CanUseItem( item )
	...
	--> Hunger
	if (item.requiresCook == nil or item.requiresCook == true) and not ply:isCook() then 
		return false, true 
	end
...
function PANEL:FillData()
	...
	if !istable(FoodItems) then FoodItems = {} end
	...
	for k, v in ipairs( FoodItems ) do
		...
		local objectEnergy 	= v.energy 			or 0
		...
		object.Paint = function( pnl, w, h )
			...
			draw.DrawText( "Energy: " .. objectEnergy, "FG_JaapokkiRegular_24", 135 + ( object:GetWide() - 155 ) / 2, 35 + 40, Color( 100, 100, 100, 255 ), 1 )
...
--[[---------------------------------------------------------
	Name: Define
-----------------------------------------------------------]]
derma.DefineControl( "FG-F4Menu-Food", "FG-F4Menu-Food", PANEL, "FG-DScrollPanel" )