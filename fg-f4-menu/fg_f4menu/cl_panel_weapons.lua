...
local function CanUseItem( item )
	...
	--> Checks
	if GAMEMODE.Config.restrictbuypistol and istable( item.allowed ) and table.Count(item.allowed) != 0 and not table.HasValue( item.allowed, ply:Team() ) then
		return false, false
	end
	...
	if not ply:canAfford( item.pricesep ) then
		return true, true
	end
...
function PANEL:FillData()
	...
	for k, v in pairs( fn.Filter(fn.Curry(fn.GetValue, 2)("seperate"), CustomShipments) ) do
		...
		local objectPrice 	= v.pricesep  		or 0
...
--[[---------------------------------------------------------
	Name: Define
-----------------------------------------------------------]]
derma.DefineControl( "FG-F4Menu-Weapons", "FG-F4Menu-Weapons", PANEL, "FG-DScrollPanel" )